...
//        this.orm = useService("orm");
    ...
    async onWillStart() {
        ...
        console.log(this.env.pos.pos_session.config_id[0]);

        this.pos_id2 = this.env.pos.pos_session.config_id[0];
        this.branch = await this.rpc({
              model: 'pos.config',
              method: 'search_read',
              args: [[['id', '=', this.pos_id2]], ['branch_id']],
            });

        console.log('this.branch')
        console.log(this.branch[0]['branch_id'][0])

        this.partners = await this.rpc({
              model: 'res.users',
              method: 'search_read',
              args: [[['is_delivery', '=', true],['branch_ids','in',this.branch[0]['branch_id'][0]]], ['name']],
            });
        }
...
export default DeliveryOrderWidget;
